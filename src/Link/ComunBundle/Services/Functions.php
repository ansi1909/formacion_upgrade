...
class Functions
{
  ...
  public function sendMailNotificationsMuro($tutores, $yml, $muro, $categoria, $tipoMensaje, $background, $logo, $link_plataforma, $usuario_padre_id = 0)
  {
    ...
    foreach ($tutores as $tutor) {
      ...
      $query = $em->createQuery('SELECT count(np.id) FROM LinkComunBundle:CertiNivelPagina np
        ...
        ->setParameters(array(
          'pagina_id' => $categoria['programa_id'],
      ...
      if (($muro->getUsuario()->getId() != $tutor->getId() && $nivel_asignado)  &&  ($correo) && ($tutor->getId() != $usuario_padre_id)) {
        ...
        $parametros_correo = [
          ...
          'datos' => [
            'leccion' => $muro->getPagina()->getNombre(),
  ...
  public function subPaginas($pagina_id, $paginas_asociadas = array(), $json = 0, $movimiento = array())
  {
    ...
    $subpages = $em->getRepository('LinkComunBundle:CertiPagina')->findBy(
      array('pagina' => $pagina_id),
      array('orden' => 'ASC')
    );
    ...
    foreach ($subpages as $subpage) {
      ...
      if (!$json) {
        ...
        if ($subPaginas['tiene'] > 0) {
          $return[] = array(
            'text' => $subpage->getCategoria()->getNombre() . ': ' . $subpage->getNombre(),
  ...
  public function subPaginasEmpresa($pagina_id, $empresa_id, $json = 0)
  {
    ...
    foreach ($subpages as $subpage) {
      ...
      if (!$json) {
        ...
        if ($subPaginas['tiene'] > 0) {
          $return[] = array(
            'text' => $subpage->getPagina()->getCategoria()->getNombre() . ': ' . $subpage->getPagina()->getNombre(),
            'state' => array('opened' => true),
            'icon' => 'fa fa-angle-double-right',
            'children' => $subPaginas['return']
          );
  ...
  public function subPaginasNivel($pagina_id, $estatus_contenido, $empresa_id)
  {
    ...
    foreach ($subpages as $subpage) {
      ...
      $query = $em->createQuery('SELECT COUNT(p.id) FROM LinkComunBundle:CertiPrueba p
        ...
        ->setParameters(array(
          'activo' => $estatus_contenido,
          'pagina_id' => $subpage->getPagina()->getId()
        ));
      $tiene_evaluacion = $query->getSingleScalarResult();

      $subpaginas[$subpage->getPagina()->getId()] = array(
        'id' => $subpage->getPagina()->getId(),
        'orden' => $orden,
  ...
  public function contenidoLecciones($pagina_arr, $wizard, $usuario_id, $yml, $empresa_id)
  {
    ...
    if ($pagina_arr['prelacion']) {
      ...
      $query = $em->createQuery('SELECT COUNT(pl.id) FROM LinkComunBundle:CertiPaginaLog pl
                                  ...
                                  AND pl.estatusPagina = :completada')
        ->setParameters(array(
          'pagina_id' => $pagina_arr['prelacion'],
          'usuario_id' => $usuario_id,
    ...
    foreach ($pagina_arr['subpaginas'] as $subpagina_arr) {
      ...
      if ($subpagina_arr['prelacion']) {
        ...
        $query = $em->createQuery('SELECT COUNT(pl.id) FROM LinkComunBundle:CertiPaginaLog pl
                                    ...
                                    AND pl.estatusPagina = :completada')
          ->setParameters(array(
            'pagina_id' => $subpagina_arr['prelacion'],
            'usuario_id' => $usuario_id,
            'completada' => $yml['parameters']['estatus_pagina']['completada']
          ));
        $leccion_completada = $query->getSingleScalarResult();
  ...
  public function calculoAvance($indexedPages, $pagina_id, $usuario_id, $yml, $puntos = 0)
  {
    ...
    if ($indexedPages[$pagina_id]['padre']) {
      ...
      $pagina_padre_log = $em->getRepository('LinkComunBundle:CertiPaginaLog')->findOneBy(array(
        'pagina' => $pagina_padre_id,
        'usuario' => $usuario_id
      ));
      ...
      if ($pagina_padre_log) {
        ...
        foreach ($indexedPages[$pagina_padre_id]['subpaginas'] as $subpagina) {
          $subpagina_log = $em->getRepository('LinkComunBundle:CertiPaginaLog')->findOneBy(array(
            'pagina' => $subpagina['id'],
            'usuario' => $usuario_id
          ));
          if ($subpagina_log) {
        ...
        if ($avance_total >= 100) {
          ...
          if (count($pruebas_logs) == count($pruebas_total) && $paginaEmpresa->getRanking()) {
            ...
            if (!$medallaUsuario) {
              $medalla = $em->getRepository('LinkComunBundle:AdminMedallas')->find($yml['parameters']['medallas']['super_smart']);
          ...
          foreach ($pruebas_logs as $prueba) {
            ...
            if ($prueba->getEstado() == $yml['parameters']['estado_prueba']['aprobado'] && $prueba->getNota() == '100.00') {
              $contador = $contador + 1;
            }
          }
          ...
          if ($contador == count($pruebas_total) && $paginaEmpresa->getRanking()) {
            ...
            if (!$medallaUsuario) {
              $medalla = $em->getRepository('LinkComunBundle:AdminMedallas')->find($yml['parameters']['medallas']['perfeccionista']);
              $medallaUsuario = new AdminMedallasUsuario();
              $medallaUsuario->setUsuario($usuario);
              $medallaUsuario->setMedalla($medalla);
              $medallaUsuario->setPagina($pagina);
          ...
          $usuarios_activos = $query->getResult();
 ...
 public function assignWinnerMedal($parentPage,$user,$yml){
  ...
  public function likes($social_id, $entidad_id, $usuario_id)
  {
    ...
    $likes = $em->getRepository('LinkComunBundle:AdminLike')->findBy(array(
      'social' => $social_id,
  ...
  public function iniciarSesion($datos)
  {
    ...
    $query = $em->createQuery(
      'SELECT u FROM LinkComunBundle:AdminUsuario u
          ...
          if ($usuario->getEmpresa()->getId() != $datos['empresa']['id']) //validamos que el usuario pertenezca a la empresa
          {
            ...
            if (!$usuario->getEmpresa()->getActivo()) {
              ...
              if (!$roles_ok) {
                ...
                $sesiones_activas = $em->getRepository('LinkComunBundle:AdminSesion')->findBy(array(
                ...
                if (!in_array($permiso->getId(), $permisos_id)) {
                  ...
                  foreach ($subpermisos as $subpermiso) {
                    ...
                    if (!in_array($subpermiso->getId(), $permisos_id)) {
                      ...
                      $submenu[] = array(
                        'id' => $subpermiso->getAplicacion()->getId(),
                  ...
                  $menu[] = array(
                    'id' => $permiso->getAplicacion()->getId(),
  ...
  public function forosHijos($foro_id, $offset, $limit, $usuario, $social_colaborativo)
  {
    ...
    foreach ($foros as $foro_hijo) {
      ...
      $foros_nietos = array();
      $foros_nietos_bd = $em->getRepository('LinkComunBundle:CertiForo')->findBy(
        array('foro' => $foro_hijo->getId()),
  ...
  public function nextLesson($indexedPages, $pagina_id, $usuario_id, $empresa_id, $yml, $programa_id)
  {
    ...
    $pl = $em->getRepository('LinkComunBundle:CertiPaginaLog')->findOneBy(array(
      'usuario' => $usuario_id,
      'pagina' => $pagina_id
    ));
  ...
  public function reordenarSubAsignaciones($pagina_empresa)
  {
    ...
    $subpages = $em->getRepository('LinkComunBundle:CertiPagina')->findBy(
      array('pagina' => $pagina_empresa->getPagina()->getId()),
      array('orden' => 'ASC')
    );

    foreach ($subpages as $subpage) {

      $subpagina_empresa = $em->getRepository('LinkComunBundle:CertiPaginaEmpresa')->findOneBy(array(
        'pagina' => $subpage->getId(),
  ...
  public function duplicarSubPaginas($pagina_id, $pagina_padre_id, $usuario_id, $evaluacion)
  {
    ...
    $paginas = $em->getRepository('LinkComunBundle:CertiPagina')->findBy(
      array('pagina' => $pagina_id),
      array('orden' => 'ASC')
    );
  ...
  public function programaVencido($pagina_id, $empresa_id, $usuario)
  {
    ...
    $pagina_empresa = $em->getRepository('LinkComunBundle:CertiPaginaEmpresa')->findOneBy(array(
      'pagina' => $pagina_id,
      'empresa' => $empresa_id
    ));
  ...
  public function getCertificado($empresa_id, $tipo_certificado, $pagina_id)
  {
    ...
    $certificado_pagina = $em->getRepository('LinkComunBundle:CertiCertificado')->findOneBy(array(
      'empresa' => $empresa_id,
    ...
    if ($certificado_pagina) {
      ...
      $certificado_grupos = $em->getRepository('LinkComunBundle:CertiCertificado')->findOneBy(array(
        'empresa' => $empresa_id,
      ...
      if ($certificado_grupos) {
        ...
        $certificado_empresas = $em->getRepository('LinkComunBundle:CertiCertificado')->findOneBy(array(
          'empresa' => $empresa_id,
  ...
  public function getActividadesRecientes($usuario_id, $paginas_sesion, $empresa_id, $yml)
  {
    ...
    foreach ($actividadreciente_padre as $arp) {
      ...
      if (array_key_exists($arp->getPagina()->getId(), $paginas_sesion) && !array_key_exists($arp->getPagina()->getId(), $actividad_reciente)) {
        ...
        $actividad_reciente[$arp->getPagina()->getId()] = array(
          'id' => $id,
          'padre_id' => $padre_id,
