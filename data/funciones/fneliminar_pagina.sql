--- Procedimiento para eliminar una pagina de la base de datos
--- No retorna nada
CREATE OR REPLACE FUNCTION fneliminar_pagina(ppagina_id integer) RETURNS VOID AS $$
DECLARE
   hijo record;
   pregunta record;
   foro record;
...
BEGIN
   ...
   SELECT array_length(estructura, 1) INTO len;
   WHILE len > 0 LOOP
        delete from certi_pagina_log where pagina_id = estructura[len];
        delete from	certi_grupo_pagina where pagina_id = estructura[len];
        --- Eliminar muros
        delete from certi_muro where pagina_id = estructura[len] and muro_id IS NOT NULL;
        delete from certi_muro where pagina_id = estructura[len] and muro_id IS NULL;
        --- Eliminar Archivos del foro
        FOR foro IN SELECT * FROM certi_foro  WHERE pagina_id = estructura[len] AND foro_id IS NULL LOOP
        	delete from certi_foro_archivo where foro_id = foro.id;
        END LOOP;
        delete from certi_foro where pagina_id = estructura[len] and foro_id IS NOT NULL;
        delete from certi_foro where pagina_id = estructura[len] and foro_id IS NULL;
        ...
        IF prueba.id IS NOT NULL THEN
        	FOR pregunta IN SELECT * FROM certi_pregunta WHERE prueba_id = prueba.id  LOOP
			...
			END LOOP;
		    delete from certi_prueba_log where prueba_id = prueba.id;
		    delete from certi_pregunta where prueba_id = prueba.id;
		    delete from certi_opcion where prueba_id = prueba.id;
		    delete from certi_prueba where id = prueba.id;
        END IF;

        delete from certi_pagina where id = estructura[len];
   		len := len -1;
   END LOOP;
...
--SELECT * from fneliminar_pagina(1);