...
BEGIN
  ...
  EXECUTE 'COPY tmp_participante(codigo,login,nombre,apellido,fecha_registro,clave,correo_personal,competencia,pais_id,campo1,campo2,campo3,campo4,nivel_id,activo,empresa_id,transaccion) FROM '|| quote_nullable(path) ||' WITH DELIMITER ''|''';
END;
$$ LANGUAGE plpgsql;
